...
""" This module defines the types of objects which can be displayed on the map.
    ...
    The player inherits of these minimum characteristics and also have the possibility to move (what a generous 
    creator I am!). """
...
class MapObject:
    ...
    def update(self, screen):
        ...
        screen.blit(
            ...
            (self.position.to_pixels(self.position.x_index, self.position.y_index)))
        pygame.display.update()


class Player(MapObject):
    """ Child of the MapObject class. Allow the player to move on allowed positions. """
    ...
    def move(self, key, map):
        """ Determine the new move of the player and associated actions up to the context. """
        ...
        if move_authorized:
            ...
            if (new_position.x_index, new_position.y_index) in map.items:
                self.items_count += 1
                map.items.remove((new_position.x_index, new_position.y_index))
    ...
    def calculation_of_the_new_position(self, key):
        """ From the key pressed by the user, calculation of the possible new position of the player
        in the structure. """
